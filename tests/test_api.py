...
from backend import config, council, github_pr, main, single, storage
...
# ---------------------------------------------------------------------------
# Chat / Quick / Humanizer endpoint
# ---------------------------------------------------------------------------


def _fake_single(final="streamed answer", error=None, mode="chat"):
    async def _gen(**kwargs):
        yield {"type": "run_start", "mode": kwargs.get("mode", mode), "model": "m/x"}
        if error:
            yield {"type": "error", "message": error}
            return
        ...
        yield {
            ...
            "final": final,
            "model": "m/x",
            "totals": {"calls": 1, "cost_usd": 0.0003},
        }
...
def test_chat_streams_and_persists(monkeypatch):
    monkeypatch.setattr(single, "run_single", _fake_single(final="hello there"))

    response = client.post("/api/chat", json={"query": "hi", "mode": "chat"})
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/event-stream")
    ...
    conversations = client.get("/api/conversations").json()["conversations"]
    assert conversations[0]["turns"] == 1
    assert conversations[0]["mode"] == "chat"

    full = client.get("/api/conversations/{}".format(conversations[0]["id"])).json()
    assert full["turns"][0]["final"] == "hello there"
    assert full["turns"][0]["mode"] == "chat"


def test_chat_failure_is_not_persisted(monkeypatch):
    monkeypatch.setattr(single, "run_single", _fake_single(error="Mesh balance empty"))

    events = _sse_events(client.post("/api/chat", json={"query": "hi"}).text)
    assert events[-1]["type"] == "error"
    assert client.get("/api/conversations").json()["conversations"][0]["turns"] == 0


@pytest.mark.parametrize("mode", ["chat", "quick", "humanize"])
def test_all_three_single_modes_are_accepted(monkeypatch, mode):
    monkeypatch.setattr(single, "run_single", _fake_single(mode=mode))
    response = client.post("/api/chat", json={"query": "text", "mode": mode})
    assert response.status_code == 200
    assert _sse_events(response.text)[0]["mode"] == mode


def test_unknown_mode_is_rejected_by_validation():
    response = client.post("/api/chat", json={"query": "x", "mode": "telepathy"})
    assert response.status_code == 422


def test_invalid_tone_is_rejected():
    response = client.post(
        "/api/chat", json={"query": "x", "mode": "humanize", "tone": "sarcastic"}
    )
    assert response.status_code == 422


def test_humanizer_controls_reach_the_service(monkeypatch):
    captured = {}

    def spy(**kwargs):
        captured.update(kwargs)
        return _fake_single()(**kwargs)

    monkeypatch.setattr(single, "run_single", spy)
    client.post(
        "/api/chat",
        json={
            "query": "stiff text",
    ...
    assert captured["length"] == "tighten"
    assert captured["model"] == "openai/gpt-4o"


def test_chat_continues_a_conversation(monkeypatch):
    monkeypatch.setattr(single, "run_single", _fake_single(final="one"))
    client.post("/api/chat", json={"query": "first"})
    conversation_id = client.get("/api/conversations").json()["conversations"][0]["id"]

    monkeypatch.setattr(single, "run_single", _fake_single(final="two"))
    client.post(
        "/api/chat", json={"query": "second", "conversation_id": conversation_id}
    )
...
def test_config_exposes_a_model_for_every_mode():
    payload = client.get("/api/config").json()
    for key in ("chat_model", "quick_model", "humanizer_model", "chairman_model"):
        assert payload[key]
    assert payload["council_models"]
